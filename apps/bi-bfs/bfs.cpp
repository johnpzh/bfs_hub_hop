...
inline unsigned intersecting_vertex_id(
				unsigned *f_h_graph_parents, 
				unsigned *b_h_graph_parents)
{
	unsigned id = (unsigned) -1;
#pragma omp parallel for
	for (unsigned i = 0; i < NNODES; ++i) {
		if ((unsigned) -1 != f_h_graph_parents[i] && (unsigned) -1 != b_h_graph_parents[i]) {
			id = i;
		}
	}
		...
		int *is_hub)
{
	...
	// Set up for Forwards
	omp_set_num_threads(NUM_THREADS);
	int *f_h_graph_mask = (int*) malloc(sizeof(int)*NNODES);
	int *h_updating_graph_mask = (int*) malloc(sizeof(int)*NNODES);
	int *f_h_cost = (int*) malloc(sizeof(int)*NNODES);
	int *f_is_active_side = (int *) malloc(sizeof(int) * SIDE_LENGTH);
	int *is_updating_active_side = (int *) malloc(sizeof(int) * SIDE_LENGTH);
	unsigned *f_h_graph_parents = (unsigned *) malloc(sizeof(unsigned) * NNODES);
	
	memset(f_h_graph_mask, 0, sizeof(int)*NNODES);
	memset(h_updating_graph_mask, 0, sizeof(int)*NNODES);
#pragma omp parallel for num_threads(64)
	for (unsigned j = 0; j < NNODES; ++j) {
		f_h_cost[j] = -1;
	...
	f_h_cost[source] = 0;
	memset(f_is_active_side, 0, sizeof(int) * SIDE_LENGTH);
	memset(is_updating_active_side, 0, sizeof(int) * SIDE_LENGTH);
#pragma omp parallel for num_threads(64)
	for (unsigned j = 0; j < NNODES; ++j) {
		f_h_graph_parents[j] = (unsigned) -1; // means unvisited yet
	}
	f_h_graph_parents[source] = source;

	unsigned f_frontier_size = 1;
	unsigned *f_frontier = (unsigned *) malloc(sizeof(unsigned) * f_frontier_size);
	f_frontier[0] = source;
	double start_time = omp_get_wtime();
	unsigned f_out_degree = graph_degrees[source];
	bool f_last_is_dense = false;
	unsigned *new_frontier = nullptr;
	
	// Set up for Backwards
	int *b_h_graph_mask = (int*) malloc(sizeof(int)*NNODES);
	int *b_h_cost = (int*) malloc(sizeof(int)*NNODES);
	int *b_is_active_side = (int *) malloc(sizeof(int) * SIDE_LENGTH);
	unsigned *b_h_graph_parents = (unsigned *) malloc(sizeof(unsigned) * NNODES);
	
	memset(b_h_graph_mask, 0, sizeof(int)*NNODES);
#pragma omp parallel for num_threads(64)
	for (unsigned j = 0; j < NNODES; ++j) {
		b_h_cost[j] = -1;
	}
	b_h_cost[destination] = 0;
	memset(b_is_active_side, 0, sizeof(int) * SIDE_LENGTH);
#pragma omp parallel for num_threads(64)
	for (unsigned j = 0; j < NNODES; ++j) {
		b_h_graph_parents[j] = (unsigned) -1; // means unvisited yet
	}
	b_h_graph_parents[destination] = destination;

	unsigned b_frontier_size = 1;
	unsigned *b_frontier = (unsigned *) malloc(sizeof(unsigned) * b_frontier_size);
	b_frontier[0] = destination;
	unsigned b_out_degree = graph_degrees_reverse[destination];
	bool b_last_is_dense = false;
...
//#endif
	...
	free( f_h_graph_mask);
	free( h_updating_graph_mask);
	free( f_h_graph_parents);
	free( f_h_cost);
	free( f_is_active_side);
	free( is_updating_active_side);
	free( b_frontier);
	free( b_h_graph_mask);
	free( b_h_graph_parents);
	free( b_h_cost);
	free( b_is_active_side);
}
			...
			unsigned &num_hubs_max)
{
	...
	fclose(fin);
	num_hubs_max = (unsigned) (NNODES * 0.05);
...
int main( int argc, char** argv) 
{
	...
	if(argc > 5){
		...
		puts("Usage: ./bfs.out <data> <tile_size> <stripe_length> <source> <destination>");
		exit(1);
	}
	...
	unsigned num_hubs = 0;
	//Reverse
	unsigned *graph_heads_reverse = nullptr;
	unsigned *graph_tails_reverse = nullptr;
	unsigned *tile_offsets_reverse = nullptr;
	unsigned *tile_sizes_reverse = nullptr;
