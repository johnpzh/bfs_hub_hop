...
//////////////////////////////////////////////////////////////////
// Dense (bottom-up)
void mark_off_hubs_dense(
		int *h_graph_mask,
		unsigned &frontier_size,
		int *is_hub)
{
	for (unsigned i = 0; i < NNODES; ++i) {
		if (h_graph_mask[i] && is_hub[i]) {
			h_graph_mask[i] = 0;
			--frontier_size;
		}
	}
...
///////////////////////////////////////////////////////////////
// Sparse (top-down)
void mark_off_hubs_sparse(
		unsigned *frontier,
		unsigned &frontier_size,
		int *is_hub)
{
	for (unsigned i = 0; i < frontier_size; ++i) {
		unsigned vertex = frontier[i];
		if (is_hub[vertex]) {
			for (unsigned pos = i + 1; pos < frontier_size; ++pos) {
				frontier[pos - 1] = frontier[pos];
			}
			--frontier_size;
		}
	}
...
#pragma omp parallel for num_threads(64)
	...
	unsigned out_degree = graph_degrees[source];
	...
	// Calculate without expansion of hubs
	memset(h_graph_mask, 0, sizeof(int)*NNODES);
	memset(h_updating_graph_mask, 0, sizeof(int)*NNODES);
#pragma omp parallel for num_threads(64)
	for (unsigned j = 0; j < NNODES; ++j) {
...
#pragma omp parallel for num_threads(64)
	...
	frontier_size = 1;
	free(frontier);
	frontier = (unsigned *) malloc(sizeof(unsigned) * frontier_size);
	frontier[0] = source;
...
#pragma omp parallel num_threads(NUM_THREADS) private(fname, fin)
{
	...
	fclose(fin);
	// Set is_hub
	unsigned num_hubs_max = (unsigned) (NNODES * hubs_max_percent);
...
int main( int argc, char** argv) 
{
	...
	if(argc > 7){
		...
		hubs_max_percent = strtod(argv[7], &nouse);
	} else {
		puts("Usage: ./bfs.out <data> <tile_size> <stripe_length> <hops> <num_sources> <degree_threshold> <hubs_max_percent>");
		exit(1);
	}
	...
	vector< unsigned > sources;
	vector< pair<unsigned, unsigned> > y_and_x_v;
	unsigned step = (NNODES + num_sources - 1) / num_sources;
	for (unsigned source = 0; source < NNODES; source += step) {
		unsigned xi = 0;
		unsigned yi = 0;
		// BFS
		graph_prepare(
		...
		y_and_x_v.push_back(make_pair(yi, xi));
	}
	printf("%lu %lu %f\n", y_sum, x_sum, 1.0 * y_sum / x_sum);
	for (unsigned i = 0; i < sources.size(); ++i) {
		unsigned source = sources[i];
		auto ele = y_and_x_v[i];
		printf("%u %u %u %f\n", source, ele.first, ele.second, 1.0 * ele.first / ele.second);
	}
	// cleanup memory
	free( graph_heads);
	free( graph_tails);
