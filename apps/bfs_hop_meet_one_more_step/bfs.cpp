...
void graph_prepare(
		...
		int *is_hub)
{
	unsigned num_visited_vertices = 0;
	unsigned max_hop = (unsigned) -1;
	unsigned hop = 0;
	bool already_met = false;
	unsigned num_visited_for_hub = 0;
	unsigned hub_frontier_size;
...
#pragma omp parallel for num_threads(64)
	...
	++hop;
	num_visited_vertices += frontier_size;
	unsigned hub_id = if_meet_hubs_sparse(
								frontier,
								frontier_size,
								is_hub);
	if (hub_id != (unsigned) -1 && !already_met) {
		already_met = true;
		num_visited_for_hub = num_visited_vertices;
		hub_frontier_size = frontier_size;
		max_hop = hop + 1; // for one more hop and stop;
	}
	...
	if (already_met) {
		printf("source: %u visited_when_meet_hub: %u visited_one_more_hop: %u hub_frontier_size: %u expanded_hub_frontier_size: %u\n", source, num_visited_for_hub, num_visited_vertices, hub_frontier_size, num_visited_vertices - num_visited_for_hub);
	}
...
int main( int argc, char** argv) 
{
	...
	if(argc > 6){
		...
		num_sources = strtoul(argv[4], NULL, 0);
		degree_threshold = strtoul(argv[5], NULL, 0);
		num_hubs_max = strtoul(argv[6], NULL, 0);
	} else {
		puts("Usage: ./bfs.out <data> <tile_size> <stripe_length> <num_sources> <degree_threshold> <num_hubs_max>");
		exit(1);
	}
	...
	CHUNK_SIZE = 2048;
	// Re-initializing
	unsigned step = NNODES / num_sources;
	unsigned source = 0;
